...
export default function EnhancedHotelCommissions() {
  ...
  const addEnhancedCommission = async (enhancedCommission: typeof enhancedCommissionsByCountryAndCity[0]) => {
    ...
    try {
      await addEnhancedCommissionAPI.mutate(payload)
  ...
  const migrateData = () => enhancedCommissionsByCountryAndCity.forEach((enhancedCommission) => addEnhancedCommission(enhancedCommission))
