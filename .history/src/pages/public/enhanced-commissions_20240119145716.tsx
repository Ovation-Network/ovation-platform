  ...
  const migrateData = () => {
    for (let i = 0; i <= 50; i++) {
      ...
      if (enhanceCommissionsCleaned[i] === undefined) {
        console.log('No data found at index', i, 'skipping......', enhanceCommissionsCleaned[i])
        continue
      }


      if (indexOfNulls.includes(i)) {
        console.log('Null value found at index', i, 'skipping......')
        continue
      }

      addEnhancedCommission(enhanceCommissionsCleaned[i])
