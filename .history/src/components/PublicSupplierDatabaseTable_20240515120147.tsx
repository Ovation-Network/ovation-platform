            ...
            {suppliers?.map((row, i) => (
              <tr className="text-black text-md text-center" key={i}>
                ...
                <td className={i % 2 ? "bg-stone-300 p-2" : "bg-stone-100 p-2"}>
                  {row.updatedBy && 
                    <>
                      <p className="text-xs font-semibold">Updated By: {row.updatedBy}</p>
                      <p className="text-xs">on {row.updatedAt.getMonth()}/{row.updatedAt.getDate()}/{row.updatedAt.getFullYear()}</p>
                    </>}
