...
export const PublicSupplierDatabaseTable: React.FC = () => {
  ...
  useEffect(() => {
    ...
    suppliers && setMaxPage(Math.ceil(suppliers?.length / suppliersPerPage)); // calculate max page
    ...
    suppliers && setSuppliers(suppliers?.slice(startIndex, endIndex));
    

  }, [filter, search, suppliers, isLoading, currentPage, startIndex, endIndex, maxPage])

  if (isLoading || !suppliers) return <div>Loading...</div>;
