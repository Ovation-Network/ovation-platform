            ...
            {suppliers?.map((row, i) => (
              <tr className="text-black text-md text-center" key={i}>
                ...
                <td className={i % 2 ? "bg-stone-300 p-2" : "bg-stone-100 p-2"}>
                  {row.updatedBy && 
                    <>
                      <p className="text-sm font-semibold">Updated By: {row.updatedBy}</p>
                      <p className="text-sm font-semibold">on {row.updatedAt.getMonth()}/{row.updatedAt.getDate()}/{row.updatedAt.getFullYear()}</p>
                    </>}
                  <NotifyIAPopup supplierId={row.id} supplierName={row.name} supplierLocation={row.city ?? row.country} />
                </td>
              </tr>
            ))}
          </tbody> 
