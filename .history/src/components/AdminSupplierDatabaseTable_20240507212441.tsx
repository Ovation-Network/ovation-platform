...
const paginationReducer = (state: paginationState, action: { type: string, payload: paginationState }) => {
  switch (action.type) {
    case 'SET_PAGE_DATA':
      return { 
        ...action.payload
    ...
    case 'SET_NEXT_PAGE':
      if (state.page !== state.maxPages) {
        return {
          ...state,
          page: state.page + 1,
          startIndex: (state.page + 1) * state.maxItemsPerPage,
        }
      } else return state;
    case 'SET_PREV_PAGE':
      if (state.page !== 1) {
...
const initialPageState: paginationState = {
  maxItemsPerPage: 25,
  numOfSuppliers: 0,
  startIndex: 0,
  page: 1,
...
export const AdminSupplierDatabaseTable: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setMaxPages(Math.ceil((suppliers ? suppliers.length : data!.length) / maxItemsPerPage));

    // setFilteredSuppliers(undefined);
  }, [data, startIndex, endIndex, page, maxPages, suppliers]);
  
  
  if (isLoading) return <div>Loading...</div>;
  
  const updatePageData = (payload: paginationState) => dispatch({ type: 'SET_PAGE_DATA', payload });
  ...
  const handleReset = (e: React.MouseEvent) => {
    ...
    setSuppliers(chopSupplierList(data, 0, maxItemsPerPage));
  }
      ...
      <div className="p-5">
        ...
        */}
        <div className="join">
          ...
          <button className="btn join-item" disabled>Page {pageState.page} of {pageState.maxPages}</button>
          <button className="btn join-item">Button</button>
        </div>
