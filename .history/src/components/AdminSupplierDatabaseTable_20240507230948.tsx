...
export const AdminSupplierDatabaseTable: React.FC = () => {
  ...
  const choppedSupplierList = useCallback((data: typeof suppliers, startIndex: number, endIndex: number) => {
  ...
  useEffect(() => {
    ...
    if (suppliers) {
      setSuppliers(choppedSupplierList(suppliers, startIndex, endIndex));
    } else {
      setSuppliers(choppedSupplierList(data, startIndex, endIndex));
    }

    // setFilteredSuppliers(undefined);
  }, [data, startIndex, endIndex, page, maxPages, suppliers, choppedSupplierList]);
