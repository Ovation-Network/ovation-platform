...
export const Notification: React.FC<NotificationProps> = ({ supplierID, notifier, details, submissionDate}) => {
  ...
  const { data, isLoading , isError } = api.notification.resolveIssue.useMutation({
    onSuccess: () => {
      void trpcUtils.notification.getUnresolvedNotifications.invalidate()
  ...
  const handleResolveNotification = (e: React.MouseEvent<HTMLDivElement>) => {
    e.preventDefault();

    resolveNotificationAPI.mutate({ id: supplierID });
  }
  ...
  return (
    <div role="alert" className="alert">
      ...
      <div>
        ...
        <div className="btn btn-sm btn-primary" onClick={async (e) => await handleResolveNotification(e)}>RESOLVE</div>
      </div>
    </div>
  )
};