...
export const AdminSupplierDatabaseTable: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (search !== '') {
      setFilteredSuppliers(supplierCache?.filter((supplier) => filter === 'supplier' ? supplier.name.toLowerCase().includes(search.toLowerCase()) : supplier.city?.toLowerCase().includes(search.toLowerCase()) ?? supplier.country?.toLowerCase().includes(search.toLowerCase()) ));
    } else {
      setFilteredSuppliers(supplierCache);
    }
    

  }, [/* filter, search, data, isLoading, */supplierCache])
