...
export const AdminSupplierDatabaseTable: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setTotalSuppliers(currentSuppliers?.length); // set total suppliers
    ...
    <div className="min-w-full flex flex-col justify-center">
      <div className="flex justify-between mb-4">
        ...
        <div className="join ml-2">
          ...
          <select className="select select-bordered join-item first-letter:bg-white border-teal-400" value={filter} onChange={(e) => setFilter(e.target.value)}>
            <option value="supplier">Supplier</option>
            <option value="location">Location</option>
          </select>
          <div className="indicator">
            <button className="btn join-item bg-white border-teal-400">Search</button>
          </div>
        </div>
        {/* Search box and filter dropdown */}
        ...
        <div className="flex justify-center mr-2 self-center">
          <div className="flex flex-row">
            ...
            <div className="flex flex-col items-center justify-center px-2 text-sm font-medium text-gray-700">
              <p>{currentPage} of {maxPage}</p>
              <p>{totalSuppliers} Total Properties / Suppliers</p>
            </div>
        ...
        <Link href={`/admin/supplier/add`}>
          <div className="btn btn-sm bg-green-400 text-white">ADD NEW SUPPLIER / PROPERTY</div>
        </Link>
