...
type searchByToggleProps = 'name' | 'id' | 'country' | 'city' | undefined;
...
export const AdminSupplierDatabaseTable: React.FC = () => {
  ...
  const { data, isLoading } = api.supplier.getSupplierContacts.useQuery(undefined, {
    ...
    initialDataUpdatedAt() {
      return api.supplier.getSupplierContacts.useQuery().dataUpdatedAt;
    },
  })
  ...
  const [searchBy, setSearchBy] = useState<searchByToggleProps>(undefined);
  ...
  const handleSearch = (e: React.MouseEvent) => {
    ...
    setSuppliers(data?.filter((supplier) => filter === 'supplier' ? supplier.name.toLowerCase().includes(search.toLowerCase()) : supplier.city?.toLowerCase().includes(search.toLowerCase()) ?? supplier.country?.toLowerCase().includes(search.toLowerCase()) ));
  }
