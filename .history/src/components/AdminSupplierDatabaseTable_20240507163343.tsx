...
export const AdminSupplierDatabaseTable: React.FC = () => {
  ...
  const [maxPages, setMaxPages] = useState<number>(Math.ceil((suppliers?.length ?? 0) / maxItemsPerPage));
  ...
  useEffect(() => {
    ...
    setMaxPages(Math.ceil((suppliers?.length ?? 0) / maxItemsPerPage));

    // setFilteredSuppliers(undefined);
  }, [data, startIndex, endIndex, page, suppliers?.length]);
      ...
      <div className="p-5">
        ...
        */}
        <div className="join">
          ...
          <button className="btn join-item" disabled>Page {page + 1} of {Math.ceil((suppliers?.length ?? 0) / maxItemsPerPage)}</button>
          <button className="btn join-item">Button</button>
        </div>
