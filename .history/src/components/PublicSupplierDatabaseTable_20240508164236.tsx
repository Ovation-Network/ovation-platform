...
export const PublicSupplierDatabaseTable: React.FC = () => {
  ...
  const filterSuppliers = useCallback((suppliers: typeof data, filter: string, search: string) => {
    if (filter === 'supplier') {
      ...
      return suppliers?.filter(supplier => supplier.city?.toLowerCase().includes(search.toLowerCase()) ?? supplier.state?.toLowerCase().includes(search.toLowerCase()) ?? supplier.country?.toLowerCase().includes(search.toLowerCase()));
    }
  }, []);
  ...
  }, [filter, search, data, isLoading, currentPage, startIndex, endIndex, maxPage])
    ...
    {/* TABLE CONTAINING DATA */}
      ...
      <table className="table-fixed w-full min-h-full">
        ...
        <tbody>
          ...
          {suppliers?.length === 0 &&  <div className="flex w-screen h-60 m-auto justify-center align-middle items-center text-black font-semibold text-lg">No results found....</div>}
          {suppliers?.map((row, i) => (
