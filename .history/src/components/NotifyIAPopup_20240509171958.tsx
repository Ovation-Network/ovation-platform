...
export const NotifyIAPopup: React.FC<NotificationProps> = ({ supplierId, supplierName, supplierType }) => {
  ...
  return (
    ...
    {/* You can open the modal using document.getElementById('ID').showModal() method */}
      <button className="btn bg-[#f8ff3796] btn-sm text-slate border-[#c3d42d]" onClick={openModal}>FLAG</button>
        ...
        <div className="modal-box bg-white">
          <form id="notifyIAForm" method="dialog bg-white">
            <div>
              ...
              <input name={`${id}-form-name`} type="text" id="name" className="shadow-sm bg-gray-50 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-300 dark:placeholder-gray-400 dark:text-black dark:focus:ring-primary-500 dark:focus:border-primary-500 dark:shadow-sm-light" placeholder="Enter name here" onChange={(e) => setNotifier(e.target.value)}/>
            </div>
            <div className="sm:col-span-2">
              ...
              <textarea name={`${id}-form-details`} id="details" rows={6} className="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg shadow-sm focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-300 dark:placeholder-gray-400 dark:text-black dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Enter details here...." onChange={(e) => setDetails(e.target.value)}></textarea>
            </div>
            {/* if there is a button in form, it will close the modal */}
          </form>
          <button className="btn btn-sm btn-circle btn-ghost absolute right-2 top-2" onClick={closeModal}>✕</button>
          <button form="notifyIAForm" type="button" className="btn mt-3 bg-[#2dd4bf] btn-sm text-white border-[#2dd4bf]" onClick={(e) => notifyIA(e)}>NOTIFY</button>
