...
export const AdminSupplierDatabaseTable: React.FC = () => {
  ...
  const [maxPages, setMaxPages] = useState<number>(suppliers ? Math.ceil(suppliers.length / maxItemsPerPage) : Math.ceil(data!.length / maxItemsPerPage));
  ...
  const filterSupplierList = useCallback((searchBy: searchByToggleProps, search: string) => {
    ...
    if (searchBy === 'name') { 
      setSuppliers(data?.filter((supplier) => supplier.name.toLowerCase().includes(search.toLowerCase())))
    } else {
      setSuppliers(data?.filter((supplier) => supplier.city?.toLowerCase().includes(search.toLowerCase()) ?? supplier.country?.toLowerCase().includes(search.toLowerCase())));
    }
  }, [data]);
        ...
        */}
        ...
        <div>
          <table className="table-fixed">
            ...
            <tbody>
              {suppliers?.map((row, i) => i >= startIndex && i < endIndex && (
