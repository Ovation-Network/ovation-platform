...
export function SupplierScrollContainer({
  suppliers,
  isError,
  isLoading,
  fetchNewSuppliers,
    ...
    <tbody>
      ...
      >
        {suppliers.map((supplier, i) => {
          return <SupplierRowItem key={`${i}-${supplier.name}-collapse-item`} {...supplier, iteration: i} />;
        })}
      </InfiniteScroll>
    </tbody>
  );
...
function SupplierRowItem({
  ...
  return (
    <tr className="text-black text-md text-center" key={i}>
      ...
      <td className={i % 2 ? "bg-stone-300 p-2" : "bg-stone-100 p-2"}>{name}</td>
      <td className={i % 2 ? "bg-stone-300 p-2" : "bg-stone-100 p-2"}>
