...
export const AdminSupplierDatabaseTable: React.FC = () => {
  ...
  // const suppliersCacheData = await trpcUtils.supplier.infiniteSupplierFeed.prefetchInfinite({}, { queryKey: ['supplier.infiniteSupplierFeed', {}]});
  // const suppliersCacheData = await trpcUtils.supplier.infiniteSupplierFeed.fetchInfinite({}, { queryKey: ['supplier.infiniteSupplierFeed', {}]})
  // console.log(suppliersCacheData)
  ...
  const { data, isLoading } = api.supplier.infiniteSupplierFeed.useInfiniteQuery({})
  const suppliers = data?.pages.flatMap((page) => page.supplier);

  const [filter, setFilter] = useState<string>('supplier');
  const [search, setSearch] = useState<string>('');
  const [filteredSuppliers, setFilteredSuppliers] = useState<typeof suppliers | undefined>(undefined);

  useEffect(() => {
    // flatten the data returned from the cache
    const suppliers = data?.pages.flatMap((page) => page.supplier);
  ...
  const handleSearch = (e: React.MouseEvent) => {
    ...
    setFilteredSuppliers(suppliers?.filter((supplier) => filter === 'supplier' ? supplier.name.toLowerCase().includes(search.toLowerCase()) : supplier.city?.toLowerCase().includes(search.toLowerCase()) ?? supplier.country?.toLowerCase().includes(search.toLowerCase()) ));
  }
