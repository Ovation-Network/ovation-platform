...
export const AddSupplierForm: React.FC = () => {
  ...
  const addSupplierAndContactsAPI = api.supplier.createSupplierAndContacts.useMutation({
    onSuccess: async () => {
      ...
      await trpcUtils.supplier.getSupplierContacts.invalidate({})
        .then(() => console.log('invalidated getSupplierContactsCache! Public page should be updated with changes'))
        .catch(() => console.log('Failed to invalidate cache, public data might serve stale data. Please ping me on Zoom to fix'));
