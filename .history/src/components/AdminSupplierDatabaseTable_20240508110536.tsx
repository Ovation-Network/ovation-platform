...
export const AdminSupplierDatabaseTable: React.FC = () => {
  ...
  const filterSupplierList = useCallback((searchBy: searchByToggleProps, search: string) => {
    ...
    if (searchBy === 'name') { 
      setSuppliers(data?.filter((supplier) => supplier.name.toLowerCase().includes(search.toLowerCase())).slice(startIndex, endIndex));
    } else {
      setSuppliers(data?.filter((supplier) => supplier.city?.toLowerCase().includes(search.toLowerCase()) ?? supplier.country?.toLowerCase().includes(search.toLowerCase())).slice(startIndex, endIndex));
    }
  }, [data, startIndex, endIndex]);
  ...
  const handleSupplierSearch = useCallback((e: React.MouseEvent) => {
    ...
    setPage(1);
    setStartIndex(0);
    setEndIndex(maxItemsPerPage);
    filterSupplierList(searchBy, search);
  }, [search, searchBy, filterSupplierList]);

  useEffect(() => {
    getPageSuppliers(page);
  }, [page, getPageSuppliers]);
        ...
        <div className="flex justify-evenly">
          ...
          <div className="join">
            ...
            <div className="indicator">
              ...
              <button className="btn join-item" onClick={(e) => handleSupplierSearch(e)}>Search</button>
              <button className="btn join-item" onClick={(e) => handleReset(e)}>Reset</button>
            </div>
          </div>
          
          <div className="join">
            <button className="btn join-item" onClick={() => page > 1 && setPage((currentPage) => currentPage - 1)} disabled={page === 1}>PREV</button>
            <button className="btn join-item" disabled>Page {page} of {maxPages}</button>
            <button className="btn join-item" onClick={() => setPage((currentPage) => currentPage + 1)} disabled={page === maxPages}>NEXT</button>
          </div>
