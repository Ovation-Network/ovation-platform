...
export const PublicSupplierDatabaseTable: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (search !== '') {
      ...
      setSuppliers(chopSuppliers(suppliers, startIndex, endIndex));

      setLoading(false);


    } else {
      setSuppliers(chopSuppliers(data, startIndex, endIndex));
      setLoading(false);
    }
    

  }, [filter, search, data, isLoading, currentPage, startIndex, endIndex, suppliers, maxPage, loading])
