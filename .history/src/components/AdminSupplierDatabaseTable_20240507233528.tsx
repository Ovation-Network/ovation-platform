...
export const AdminSupplierDatabaseTable: React.FC = () => {
  ...
  const [maxPages, setMaxPages] = useState<number>(Math.ceil(data!.length / maxItemsPerPage));
  ...
  const filterSupplierList = useCallback((searchBy: searchByToggleProps, search: string) => {
    ...
    if (searchBy === 'name') {
      return setSuppliers(data?.filter((supplier) => supplier.name.toLowerCase().includes(search.toLowerCase())))
    } else {
      return setSuppliers(data?.filter((supplier) => supplier.city?.toLowerCase().includes(search.toLowerCase()) ?? supplier.country?.toLowerCase().includes(search.toLowerCase())));
    }
  }, [data]);

  const handleSupplierSearch = useCallback((e: React.MouseEvent) => {
    e.preventDefault();
    setSuppliers(filterSupplierList(searchBy, search));
